...
def user_chosen_menu_option(user_option,source_file_list,source_csv):
    user_option=int(user_option)
    
    if(user_option==1):
        updated_list=add_new_student(source_csv,source_file_list)
...
#DONT WANT TO PUT THE PRINT FORMAT IN THAT FUNCTION BUT JUST THE LIST RETURNED WITH THE RAW DATA AND WE FORMAT HERE
        ...
        list_all_students=get_all_students(source_file_list)
        ...
        for item in list_all_students:
            ...
            print(f"Average Grade: {item.avg_grade}")
            print('-' * 30) 
            
        menu_prompt(list_all_students)
...
#DONT WANT TO PUT THE PRINT FORMAT IN THAT FUNCTION BUT JUST THE LIST RETURNED WITH THE RAW DATA AND WE FORMAT HERE
        ...
        list_of_top_students=(get_top_3_students(source_file_list))
        ...
        menu_prompt(source_file_list)

#THIS OPTION CALLS THE GET_GROUP_AVERAGE_GRADE() FUNCTION IN ACTION.PY , 
#DONT WANT TO PUT THE PRINT FORMAT IN THAT FUNCTION BUT JUST THE LIST RETURNED WITH THE RAW DATA AND WE FORMAT HERE

    if(user_option==4):
        print(f"\n\n The group's overall average grade is: [{get_group_average_grade(source_file_list)}]")
        menu_prompt(source_file_list)


    if(user_option==5):
        export_students_list_to_file(source_csv,source_file_list)
        menu_prompt(source_file_list)
